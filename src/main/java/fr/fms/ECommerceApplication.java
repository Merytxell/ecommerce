...
public class ECommerceApplication implements CommandLineRunner {
	...
	public static void displayArticle(ArticleRepository articleRepository) {
		System.out.println(Article.centerString(COLUMN_ID) + Article.centerString(COLUMN_BRANDOFARTICLE)
	...
	public void run(String... args) throws Exception {
		...
		while (choice != 12) {
			...
			switch (choice) {
			case 1:
				displayArticle(articleRepository);
				break;
			case 2:
				System.out.println("Saisissez le numéro de page que vous souhaitez consulter");
				int pageSize = 5;
				int currentPage = 1;
				...
				else if (userChoice .equals("PAGE")) {
					System.out.println("entrez le nombre de page (par défaut c'est 5)");
					int articlePerPageNo = Integer.parseInt(scanner.nextLine());
					pageSize = articlePerPageNo;
					currentPage=0;
					page = articleRepository.findAll(PageRequest.of(currentPage, pageSize));
					page.forEach(article -> System.out.println(article.toString()));
					displayPagination(totalPages, currentPage);
			...
			case 3:
				...
				System.out.println(
						"Insérer l'id category correspondant : 1 : smartphone, 2 : tablet, 3: pc, 4 : keyboard,  5 : screen, 6 : print, 7: console");
	...
	/*
	 * }else { System.out.println(Article.centerString(COLUMN_ID) +
	 * Article.centerString(COLUMN_BRANDOFARTICLE) +
	 * Article.centerString(COLUMN_DESCRIPTION) +
	 * Article.centerString(COLUMN_PRICE)); for (Article article : page) {
	 * System.out.println(article.centerString());
	 */
